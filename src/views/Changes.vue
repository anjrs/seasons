// 1. Modifiez la structure des données pour stocker les listes de prix
data() {
  return {
    products: [],
    ...
    panier: [],
    priceLists: [],  // Pour stocker toutes les listes de prix
    productPrices: {},
...
// 2. Mettez à jour l'initialisation pour récupérer d'abord les listes de prix
async initializeData() {
  this.loading = true;
  try {
    // Charger d'abord les listes de prix
    await this.fetchPriceLists();
    
    // Charger les produits de base
    await this.fetchProducts();
    
    // Charger les données de prix pour chaque liste de prix
    if (this.priceLists.length > 0) {
      const priceListPromises = this.priceLists.map(list => 
        this.fetchProductPrices(list.id)
      );
...
// 3. Ajouter une méthode pour récupérer les listes de prix
async fetchPriceLists() {
  try {
    ...
    this.priceLists = response.data.records || [];
    console.log(`Récupération de ${this.priceLists.length} listes de prix`);
  } catch (error) {
...
// 4. Mettez à jour la méthode de filtrage des produits pour utiliser activePriceList
selectPriceList(priceListId) {
  this.activePriceList = priceListId;
}

// 5. Mettre à jour le computed filteredProducts
// Remplacez le computed filteredProducts par celui-ci
filteredProducts() {
  ...
  // Filtrer par liste de prix active
  if (this.activePriceList !== null) {
    result = result.filter(product => {
      // Vérifier si le produit a un prix dans la liste de prix active
  ...
  result = result.map(product => {
    ...
    if (this.activePriceList !== null && 
      ...
      productCopy.currentPrice = priceInfo.PriceStd || priceInfo.PriceList;
      productCopy.originalPrice = priceInfo.PriceList;
    } else {
      productCopy.currentPrice = product.PriceStd;
      productCopy.originalPrice = product.PriceList;
    }
